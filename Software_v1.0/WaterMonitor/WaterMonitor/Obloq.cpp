...
{
	String wifi = (String)F("{\"type\":\"system\",\"SSID\":\"");
	wifi.concat(this->_ssid);
	wifi.concat(F("\",\"PWD\":\""));
	wifi.concat(this->_pwd);
	wifi.concat(F("\"}"));
...
void Obloq::connect(const String& client_id,const String& iot_id,const String& iot_pwd,const String& url,const String& port)
{
	...
	String connect_msg = (String)F("{\"type\":\"mqtt\",\"method\":\"connect\",\"ClientId\":\"");
	connect_msg.concat(client_id);
	connect_msg.concat(F("\",\"Iot_id\":\""));
	connect_msg.concat(iot_id);	
	connect_msg.concat(F("\",\"Iot_pwd\":\""));
	connect_msg.concat(iot_pwd);	
	connect_msg.concat(F("\",\"url\":\""));
	connect_msg.concat(url);
	connect_msg.concat(F("\",\"port\":\""));
	connect_msg.concat(port);
	connect_msg.concat( F("\"}"));
...
void Obloq::subscribe(const String& topic)
{
	...
	String subscribe_msg = (String)F("{\"type\":\"mqtt\",\"method\":\"subscribe\",\"topic\":\"");
	subscribe_msg.concat(topic);
	subscribe_msg.concat(F("\"}"));
...
void Obloq::publish(const String& topic, const String &message)
{
	...
	String publish_msg = (String)F("{\"type\":\"mqtt\",\"method\":\"publish\",\"topic\":\"");
	publish_msg.concat(topic);
	publish_msg.concat(F("\",\"message\":\""));
	publish_msg.concat(message);
	publish_msg.concat(F("\"}"));
